...
func TestCacheWrap(t *testing.T) {
	c := NewWithConfig(Config{MaxBytes: bucketsCount * chunkSize * 1.5})
	defer c.Reset()
...
func TestCacheSetGetSerial(t *testing.T) {
	itemsCount := 10000
	c := NewWithConfig(Config{MaxBytes: 30 * itemsCount})
	defer c.Reset()
...
func TestCacheGetSetConcurrent(t *testing.T) {
	...
	c := NewWithConfig(Config{MaxBytes: 30 * itemsCount * goroutines})
	defer c.Reset()
...
func TestCacheResetUpdateStatsSetConcurrent(t *testing.T) {
	c := NewWithConfig(Config{MaxBytes: 12334})
