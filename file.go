...
//
// Deprecated: use LoadFromFileWithConfig instead.
func LoadFromFile(filePath string, maxBytes int) (*Cache, error) {
	// Use default hasher for legacy load function
	// If the cache file is generated by other hasher, it's caller's
	// duty to ensure the consistency.
	return load(filePath, maxBytes, xxhasher{})
}

// LoadFromFileWithConfig loads cache data from the given filePath.
func LoadFromFileWithConfig(filePath string, config Config) (*Cache, error) {
...
//
// Deprecated: use LoadFromFileOrNewWithConfig instead.
func LoadFromFileOrNew(filePath string, maxBytes int) *Cache {
	// Use default hasher for legacy load function
	// If the cache file is generated by other hasher, it's caller's
	// duty to ensure the consistency.
	c, err := load(filePath, maxBytes, xxhasher{})
	if err == nil {
		return c
	}
	return New(maxBytes)
...
// LoadFromFileOrNewWithConfig tries loading cache data from the given filePath.
//
// The function falls back to creating new cache with the given cache config
// if error occurs during loading the cache from file.
func LoadFromFileOrNewWithConfig(filePath string, config Config) *Cache {
	// Don't panic for lazy user who doesn't specify the hasher.
	if config.Hasher == nil {
		config.Hasher = xxhasher{}
	}
